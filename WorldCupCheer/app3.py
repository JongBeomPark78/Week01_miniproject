...
def comment_get():
    try:
        ...
        return jsonify({'comments': comment_list, 'token_id': userinfo['id']})
    except jwt.ExpiredSignatureError:
        comment_list = list(db.Comments.find({}, {'_id': False}))
...
def comment_delete():
    try:
        ...
        comments_count_receive = request.form['comments_count']

        if (db.Comments.find_one({'comments_count': int(comments_count_receive), 'userid': userinfo['id']})):
            db.Comments.delete_one({'comments_count': int(comments_count_receive), 'userid': userinfo['id']})
            return jsonify({})
        else:
...
def id_overlap_register():
    id_receive = request.form['id_give']
    email_list = db.user.find_one({'id':id_receive}, {'_id': False})
    print(email_list)
    if email_list:
        return jsonify({'result': 'fail'})
    else:
        return jsonify({'id':id_receive})


# NiCKNAME 중복확인임
@app.route('/api/nick_overlap',methods=['POST'])
def nick_overlap_register():
    usernick_receive = request.form['usernick_give']
    usernick_list = db.user.find_one({'nick': usernick_receive}, {'_id': False})
    print(usernick_list)
    if usernick_list:
        return jsonify({'result': 'fail'})
    else:
        return jsonify({'nick': usernick_receive})
...
def athlete_like_post():
    ...
    try:
        ...
        result = db.Like_list.find_one({'id': userinfo['id'], 'Card_number': int(number_receive)})
        print(result)
        Like_Count = db.player.find_one({"Card_number": int(number_receive)})
        print(Like_Count['Like_Count'])
        if (result):
            db.player.update_one({'Card_number': int(number_receive)},
                                 {'$set': {'Like_Count': Like_Count['Like_Count'] - 1}})
            db.Like_list.delete_one({'id': userinfo['id'], 'Card_number': int(number_receive)})
